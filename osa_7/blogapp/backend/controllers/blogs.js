...
blogsRouter.delete(
  ...
  async (request, response) => {
    ...
    if (user.id !== blog.user.toString()) {
      return response
        .status(403)
        .json({ error: 'user does not have permission to delete the resource' })
    }
...
blogsRouter.put('/:id', middleware.userExtractor, async (request, response) => {
  ...
  if (user.id !== blog.user.toString()) {
    return response
      .status(403)
      .json({ error: 'user does not have permission to update the resource' })
  }
...
blogsRouter.post('/:id/comments', middleware.userExtractor, async (request, response) => {
  ...
  // Laitoin onnistuneen päivittämisen status-koodiksi 201:
  const update = await Blog.findByIdAndUpdate(request.params.id, updatedBlog, {
    new: true,
  })
